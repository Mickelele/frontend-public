...
export async function createLessonsForGroup(groupId, options = {}) {
    return await apiFetch(`/zajecia/${groupId}/zajecia/auto-create`, {
        method: 'POST',
        body: {
            assignRoom: options.assignRoom !== false,
            Sala_id_sali: options.Sala_id_sali || null
        }
    }, 'course');
}

export async function updateRoomForGroupLessons(groupId, newRoomId) {
    return await apiFetch(`/zajecia/${groupId}/zajecia/update-room`, {
        method: 'PATCH',
        body: { Sala_id_sali: newRoomId }
    }, 'course');
}
